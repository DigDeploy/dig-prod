...
<script>
import { unallocatedUserList, joushiUserSelectAll,deptTreeSelect } from "@/api/system/user";
export default {
  ...
  methods: {
    ...
    handleSelectUser() {
      ...
      if (userIds == "") {
        this.$modal.msgError("割り当てるユーザーを選択してください");
        return;
      }
      joushiUserSelectAll({ userId: userId, joushiIds: userIds }).then(res => {
        this.$modal.msgSuccess("ユーザー追加しました。");
        if (res.code === 200) {
          this.visible = false;
          this.$emit("ok");
        }
