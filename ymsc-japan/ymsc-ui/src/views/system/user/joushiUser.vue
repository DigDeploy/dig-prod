...
<!--      </el-table-column>-->
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-circle-close"
            @click="cancelJoushiUser(scope.row)"
            v-hasPermi="['system:role:remove']"
          >解除</el-button>
        </template>
      </el-table-column>
    ...
    <pagination
      ...
      @pagination="getList"
    />
    <select-user ref="select" :userId="queryParams.userId" @ok="handleQuery" />
  </div>
</template>

<script>
import { allocatedUserList, joushiUserCancel, joushiUserCancelAll } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    /** ロール解除ボタン操作 */
    cancelJoushiUser(row) {
      const userId = this.queryParams.userId;
      this.$modal.confirm('選択したユーザーの上司設定を解除してもよろしいですか？').then(function() {
        return joushiUserCancel({ joushiId: row.userId, userId: userId });
      }).then(() => {
        this.getList();
    ...
    /** 一括ロール解除ボタン操作 */
    cancelJoushiUserAll(row) {
      const userId = this.queryParams.userId;
      const joushiIds = this.userIds.join(",");
      this.$modal.confirm('選択したユーザーの上司設定を解除してもよろしいですか？').then(function() {
        return joushiUserCancelAll({ userId: userId, joushiIds: joushiIds });
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("解除しました。");
      }).catch(() => {});
