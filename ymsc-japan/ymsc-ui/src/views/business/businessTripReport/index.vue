    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['business:businessTripReport:add']"
          >追加</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['business:businessTripReport:export']"
          >エクスポート</el-button
        >
      </el-col> -->
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['business:businessTripReport:edit']"
            >編集</el-button
          >
          <el-button
            size="mini"
          ...
          <el-button
            ...
            v-hasPermi="['business:businessTripReport:remove']"
            >削除</el-button
          >
        </template>
      </el-table-column>
...
<script>
import {
  listBusinessTripReport,
  delBusinessTripReport,
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBusinessTripReport(this.queryParams).then((response) => {
        this.businessTripReportList = response.rows;
    ...
    handleDetail(row) {
      // TODO: 手動でルートを追加
      const router =
        "/businessTrip/businessTripReport/businessTripReportDetail/" +
        row.reportNo;
    ...
    handleAdd() {
      // TODO: 手動でルートを追加
      const router =
        "/businessTrip/businessTripReport/businessTripReportAdd/index";
      this.$router.push(router);
    },
    ...
    handleUpdate(row) {
      const router =
        "/businessTrip/businessTripReport/businessTripReportEdit/" + row.reportNo;
      this.$router.push(router);
    },
    /** 削除ボタン操作 */
    handleDelete(row) {
      ...
      that.$modal
        ...
        .then(function () {
          that.$modal.loading();
          delBusinessTripReport(reportNos)
            .then((response) => {
              that.getList();
    ...
    handleExport() {
      this.download(
        "business/businessTripReport/export",
        {
          ...this.queryParams,
        },
        `businessTripReport_${new Date().getTime()}.xlsx`
      );
    },
  },
};
