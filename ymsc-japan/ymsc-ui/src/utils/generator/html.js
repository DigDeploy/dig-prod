...
export function dialogWrapper(str) {
  return `<el-dialog v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="ダイアログタイトル">
    ${str}
    <div slot="footer">
...
// span不为24的用el-col包裹（ロジック説明のため原中文を維持）
...
// el-input innerHTML（ロジック説明のため原中文を維持）
...
function buildElUploadChild(conf) {
  ...
  if (conf.showTip) list.push(`<div slot="tip" class="el-upload__tip">${conf.accept}ファイルのみアップロード可能で、サイズは${conf.fileSize}${conf.sizeUnit}以下に限ります</div>`)
  return list.join('\n')
}
