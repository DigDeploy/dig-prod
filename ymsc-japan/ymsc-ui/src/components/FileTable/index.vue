    ...
    >
      ...
      >
        <template #default="scope">
          <template v-if="isImageOrPdf(scope.row.name)">
            <a :href="getFullUrl(scope.row.url)" target="_blank" style="color: #409eff; text-decoration: none" @click.stop>
              {{ scope.row.name }}
            </a>
          </template>
          <template v-else>
    ...
    export default {
        ...
        methods: {
           isImageOrPdf(fileName) {
              ...
              const pdfExt = 'pdf';
              return imageExts.includes(ext) || ext === pdfExt;
            },
            getFullUrl(url) {
            ...
            downloadFile() {
                ...
                this.multipleSelections.forEach(el => {
                    if (el.localUrl) {
                        ...
                        // download関数はYMSC標準機能で、1番目のパラメータはリクエストURL（変更不要）、対応するControllerメソッドは若依で事前に封装されている
            ...
            handleDelete(row, index) {
              this.$modal.confirm("選択したファイルを削除してもよろしいですか？") .then(() => { 
